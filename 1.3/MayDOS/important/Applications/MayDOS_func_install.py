...
def sof(cmd):
    try:
        if cmd.endswith('json'):
            ...
            if os.path.isfile(f'important/Applications/{cmd[4:-1]}/{cmd[4:-1]}.json') == False or os.path.isfile(
                    f'important/Applications/{cmd[4:-1]}/{cmd[4:-1]}/MAIN.txt') == False:
                # 如果文件不是在以上两个文件夹里面的，就判定为被《恶意纂改》
                match input(
                    f'{Font.YELLOW}MayDos/Root/SOF>>>应用程序"{cmd[4:-1]}"可能已被恶意篡改,请谨慎运行。（Y/N){Font.WHITE}').lower():
                    case "y" | "yes":
                        try:
